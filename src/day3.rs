pub fn part1(input: &str) -> u32 {
    ...
    let mut p = 0;
    loop {
        if let Some(n) = memchr::memchr(109, &d[p..]) {
            p += n;
        }
        if p > d.len() - 5 {
            break;
        }
        match (d[p], d[p + 1], d[p + 2], d[p + 3]) {
            (109, 117, 108, 40) => {
                p += 4;
                let (x, n) = atoi_simd::parse_any_pos::<u32>(&d[p..]).unwrap_or((0, 0));
                if n == 0 {
                    continue;
                }
                p += n as usize;
                ...
                let (y, m) = atoi_simd::parse_any_pos::<u32>(&d[p..]).unwrap_or((0, 0));
                if m == 0 {
                    continue;
                }
                p += m as usize;
...
pub fn part2(input: &str) -> u32 {
    ...
    loop {
        if let Some(n) = memchr::memchr2(109, 100, &d[p..]) {
            p += n;
        }
        if p > d.len() - 5 {
            break;
        ...
        match (d[p], d[p + 1], d[p + 2], d[p + 3]) {
            ...
            (100, 111, 110, 39) => {
                p += 4;
                if p < d.len() - 3 {
                    if d[p] == 116 && d[p + 1] == 40 && d[p + 2] == 41 {
                        p += 3;
                        enabled = false;
                        if let Some(n) = memchr::memchr(100, &d[p..]) {
                            p += n;
                        } else {
                            break;
                        }
